...
const EDGE_CASES = [
  ...
  },
  {
    ...
    message:
      'Philosophy '.repeat(100) +
      'is the fundamental question about the nature of reality, truth, existence, and the meaning of life. '.repeat(
        20,
      ),
    shouldFail: false,
  ...
  },
  {
    ...
    message:
      'What do you think about {"philosophy": "wisdom", "question": "what is truth?", "answer": "unknown"}',
    shouldFail: false,
...
async function runEdgeCaseTest(testCase) {
  ...
  if (testCase.messages) {
    ...
    try {
      ...
      console.log(
        `📊 Results: ${successful.length}/${testCase.messages.length} successful in ${totalTime}ms`,
      );
    ...
    if (result.success) {
      console.log(
        `📊 Response: ${result.responseLength} chars in ${result.totalTime}ms (${result.chunkCount} chunks)`,
      );
