...
class DelegationTester {
  ...
  async testPerformance() {
    ...
    console.log(
      `Non-delegated queries - Avg: ${this.results.performance.nonDelegated.average.toFixed(0)}ms`,
    );
    console.log(
      `Overhead: ${(this.results.performance.delegated.average - this.results.performance.nonDelegated.average).toFixed(0)}ms`,
    );
  }
  ...
  async testCharacterConsistency() {
    ...
    if (result.success && result.responseText) {
      ...
      const foundMarkers = philosophicalMarkers.filter((marker) => response.includes(marker));
      ...
      console.log(
        `  Character maintained: ${characterConsistency.maintainsCharacter ? 'Yes' : 'No'}`,
      );
  ...
  generateSummary() {
    const shouldDelegatePassed =
      this.results.testResults.shouldDelegate?.filter((t) => t.passed).length || 0;
    const shouldDelegateTotal = this.results.testResults.shouldDelegate?.length || 0;

    const shouldNotDelegatePassed =
      this.results.testResults.shouldNotDelegate?.filter((t) => t.passed).length || 0;
    ...
    this.results.summary = {
      shouldDelegate: {
        ...
        percentage:
          shouldDelegateTotal > 0 ? (shouldDelegatePassed / shouldDelegateTotal) * 100 : 0,
      },
      shouldNotDelegate: {
        passed: shouldNotDelegatePassed,
        total: shouldNotDelegateTotal,
        percentage:
          shouldNotDelegateTotal > 0 ? (shouldNotDelegatePassed / shouldNotDelegateTotal) * 100 : 0,
      },
      edgeCases: {
      ...
      overallAccuracy: {
        ...
        percentage:
          shouldDelegateTotal + shouldNotDelegateTotal > 0
            ? ((shouldDelegatePassed + shouldNotDelegatePassed) /
                (shouldDelegateTotal + shouldNotDelegateTotal)) *
              100
            : 0,
      },
    };
  ...
  async run() {
    ...
    try {
      ...
      console.log('\n=== TEST SUMMARY ===');
      console.log(
        `Should delegate accuracy: ${this.results.summary.shouldDelegate.passed}/${this.results.summary.shouldDelegate.total} (${this.results.summary.shouldDelegate.percentage.toFixed(1)}%)`,
      );
      console.log(
        `Should not delegate accuracy: ${this.results.summary.shouldNotDelegate.passed}/${this.results.summary.shouldNotDelegate.total} (${this.results.summary.shouldNotDelegate.percentage.toFixed(1)}%)`,
      );
      console.log(
        `Edge cases passed: ${this.results.summary.edgeCases.passed}/${this.results.summary.edgeCases.total} (${this.results.summary.edgeCases.percentage.toFixed(1)}%)`,
      );
      console.log(
        `Overall accuracy: ${this.results.summary.overallAccuracy.passed}/${this.results.summary.overallAccuracy.total} (${this.results.summary.overallAccuracy.percentage.toFixed(1)}%)`,
      );
      ...
      if (this.results.testResults.characterConsistency) {
        console.log(
          `Character consistency: ${this.results.testResults.characterConsistency.maintainsCharacter ? 'MAINTAINED' : 'COMPROMISED'}`,
        );
      }
