...
class WebSearchTester {
  ...
  async testTriggerDetection() {
    ...
    for (const query of searchQueries) {
      try {
        const response = await this.makeRequest([{ role: 'user', content: query }]);

        // Check if response indicates search was performed
        const hasSearchIndicators =
          response.body.includes('Mock Result') ||
  ...
  async testNonTriggerQueries() {
    ...
    for (const query of philosophicalQueries) {
      try {
        ...
        this.logTest('nonTrigger', `Query: "${query}"`, false, `Request failed: ${error.message}`);
      }
    }
  }
  ...
  async testIntegrationAndCharacter() {
    ...
    try {
      const response = await this.makeRequest([{ role: 'user', content: testQuery }]);

      // Test 1: Search results should be integrated
      const hasSearchIntegration =
        response.body.includes('Mock Result') || response.body.includes('search');
      this.logTest(
        'integration',
      ...
      this.logTest(
        'character',
        'Maintains philosophical character',
        hasPhilosophicalLanguage,
        hasPhilosophicalLanguage
          ? 'Philosophical language present'
          : 'Missing philosophical perspective',
      );

      // Test 3: Should critique/question sources
      const hasCriticalThinking =
        response.body.toLowerCase().includes('question') ||
      ...
      this.logTest(
        'character',
        'Shows critical thinking about sources',
        hasCriticalThinking,
  ...
  async testTechnicalAspects() {
    ...
    try {
      ...
      this.logTest('technical', 'Basic functionality', false, `Request failed: ${error.message}`);
    }
  }
  ...
  async testEdgeCases() {
    ...
    // Test 1: Empty query
    try {
      const response = await this.makeRequest([{ role: 'user', content: '' }]);
      ...
      this.logTest('edgeCases', 'Empty query handling', false, `Failed: ${error.message}`);
    }

    // Test 2: Very long query with search triggers
    try {
      const longQuery = 'What happened today? '.repeat(50) + 'Tell me the latest news.';
      const response = await this.makeRequest([{ role: 'user', content: longQuery }]);

      this.logTest(
        'edgeCases',
        'Long query with search triggers',
        response.status === 200,
        `Handled long query with status ${response.status}`,
      );
    } catch (error) {
      this.logTest('edgeCases', 'Long query handling', false, `Failed: ${error.message}`);
    }

    // Test 3: Multiple search trigger words
    try {
      const multiTriggerQuery =
        'What are the latest news today about current Bitcoin prices and recent stock updates?';
      const response = await this.makeRequest([{ role: 'user', content: multiTriggerQuery }]);
      ...
      this.logTest('edgeCases', 'Multiple trigger words', false, `Failed: ${error.message}`);
    }
  }
