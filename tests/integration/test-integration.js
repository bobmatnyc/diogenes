...
async function testChatUI() {
  ...
  try {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const loadingExists = await page.locator('[class*="loading"], [class*="animate"]').count() > 0;
      if (loadingExists) {
    ...
    console.log(`${assistantResponseFound ? '✅' : '❌'} Assistant response received: ${assistantResponseFound ? 'Yes' : 'No'}`);
