...
async function testAntiSycophancy() {
  ...
  try {
    ...
    if (result.success && (patternsFound.length > 0 || contrarianCount >= 2)) {
      log(
        `✅ PASSED: Anti-sycophancy working (${contrarianCount} contrarian indicators)`,
        'success',
      );
      testResults.passed++;
      testResults.details.push({
  ...
  try {
    ...
    if (successCount === test.messages.length) {
      ...
      testResults.errors.push(
        `${test.name}: ${test.messages.length - successCount} messages failed`,
      );
    }
  } catch (error) {
    log(`❌ ERROR: ${error.message}`, 'error');
...
async function generateReport() {
  ...
  if (testResults.passed === testResults.total) {
    ...
    log(
      `\n❌ MAJOR ISSUES: Only ${testResults.passed}/${testResults.total} tests passed.`,
      'error',
    );
  }
}
