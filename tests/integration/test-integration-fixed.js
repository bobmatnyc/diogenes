...
async function testChatUI() {
  ...
  try {
    ...
    const assistantMessages = await page.locator('div:has-text("Diogenes")').filter({ 
    ...
    if (assistantMessages.length > 0) {
      ...
      responseText = responseText.replace(/^Diogenes\s*/, '').replace(/\d{1,2}:\d{2}:\d{2}\s*(AM|PM)\s*$/, '').trim();
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const currentMessages = await page.locator('div:has-text("Diogenes")').filter({ 
      ...
      if (currentMessages.length > assistantMessages.length) {
        ...
        if (newResponse !== lastContent && newResponse.length > lastContent.length) {
          ...
          console.log(`  - Streaming detected: ${newResponse.length} chars after ${Date.now() - startTime}ms`);
